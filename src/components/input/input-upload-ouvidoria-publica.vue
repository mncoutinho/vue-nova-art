...
export default {
  ...
  methods: {
    add (e) {
      if (this.parametros !== undefined) {
        ...
        if (!this.parametros.tipo.includes(e[0].type) || e[0].size > this.parametros.tamanho) {
          ...
          this.mostrarMensagem('Inserir um arquivo do tipo pdf e menor que 4 MB', 'negative', TIMEOUT)
        } else {
    ...
    url () {
      return process.env.API + '/rest-api/crea/commons/arquivo/publico'
    },
    download () {
